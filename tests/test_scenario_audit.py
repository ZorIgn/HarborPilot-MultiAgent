...
def test_scenario_audit_quality_gates_pass() -> None:
    ...
    for result in results:
        ...
        assert {"AssessmentAgent", "ResearchAgent", "MatchingAgent", "VerificationAgent", "CriticAgent"} <= trace_nodes
        assert "ScenarioAuditAgent" not in trace_nodes
        assert result.audit_trace[-1].node == "CriticAgent"
