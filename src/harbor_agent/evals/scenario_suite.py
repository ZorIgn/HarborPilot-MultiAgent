...
class ScenarioEvalSuite:
    """Runs deterministic quality gates over simulated runtime workflows."""

    suite_name = "ScenarioEvalSuite"
    guardrails = (
    ...
    def run(
        self,
        expectation: ScenarioEvalExpectation,
    ...
    def _check_plan(self, expectation: ScenarioEvalExpectation, plan: ProgramPlanResult, failures: list[str]) -> None:
        ...
        missing_runtime_agents = sorted(required_runtime_agents - trace_nodes)
        if missing_runtime_agents:
            failures.append(f"plan trace missing runtime agents: {missing_runtime_agents}")
