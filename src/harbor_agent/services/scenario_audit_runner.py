...
def scenario_audit_summary() -> dict:
    results = run_audit()
...
def _case_summary(result: ScenarioAuditResult) -> dict:
    ...
    return {
        ...
        "strict_intent": result.plan.intent_profile.strict_intent if result.plan.intent_profile else None,
        "application_mix_count": len(result.plan.application_mix),
        "review_passed": result.application.review.get("passed"),
        "crawl_queue": {
            "job_count": result.crawl_queue.job_count,
            "official_job_count": result.crawl_queue.official_job_count,
            "community_job_count": result.crawl_queue.community_job_count,
        },
        "targets": [
            _target_summary(result, program_id, package_by_id.get(program_id))
            for program_id in result.case.target_program_ids
        ],
        "trace_nodes": [event.node for event in result.audit_trace],
    }


def _target_summary(result: ScenarioAuditResult, program_id: str, package) -> dict:
    match = next((item for item in result.plan.recommendations if item.program.id == program_id), None)
    ...
    return {
        ...
        "formal_recommendation": match.formal_recommendation if match else None,
        "data_status": match.program.data_status.value if match else None,
        "coverage_item_count": len(package.coverage_items) if package else 0,
        "production_ready": package.production_ready if package else None,
        "review_pending_count": queue.pending_count if queue else None,
        "publishable_count": queue.publishable_count if queue else None,
    }
