...
STYLE_GUIDE = [
    ...
    "Why Program 只绑定项目官网、申请系统或官方 PDF 中可确认的信息；未核验课程、教授、就业数据和录取结果不得写入。",
...
DOCUMENT_TYPE_RULES = {
    "PS": [
        ...
        "英文稿应是可继续精修的申请陈述，不写未证实课程、教授、就业数据或录取结果。",
    ],
    "SOP": [
...
class WritingComposer:
    name = "WritingComposer"
            ...
            try:
                completion = self.llm.complete_json(
                    system=(
                        ...
                        "Do not fabricate awards, internships, professors, course names, employment outcomes, admissions outcomes or official requirements. Treat the local questionnaire templates as question structure only; never reuse template examples, case facts, or service-promise wording as student facts. "
