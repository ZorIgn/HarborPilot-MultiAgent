...
class TimelineAgent:
    ...
    def run(self, matches: list[ProgramMatch], today: date | None = None) -> list[TimelineTask]:
        ...
        # These matches come from the student's explicit final selection. Keep
        # their choice even when the assessment marks a programme as risky;
        # the timeline should surface the blockers, not silently drop it.
        ...
        for item in selected:
            ...
            tasks[f"essay_{program.id}"] = _task(
                ...
                institution=institution,
                program_name=program_name,
                program_round=program.program_round if hasattr(program, "program_round") else "主轮次",
                round_open_date=program.open_date,
                round_deadline=formal_deadline,
                application_url=application_url,
                submit_to="学校网申系统",
                source_url=source_url,
...
def _deadline_verified(match: ProgramMatch) -> bool:
    return formal_timeline_ready(match)
