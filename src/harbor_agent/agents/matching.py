...
class SchoolMatchingAgent:
    ...
    def _tier(self, strategy_band: str) -> str:
        ...
        if strategy_band in {"reach", "target", "safe", "candidate"}:
...
def _balanced_recommendation_output(
    ...
    blocked = [item for item in matches if item.tier == "not_recommended"][: int(configured_quotas.get("not_recommended", 12))]
    quotas = {band: int(configured_quotas.get(band, default)) for band, default in {"reach": 14, "target": 14, "safe": 12, "candidate": 10}.items()}
...
def _consultant_note(
    ...
    challenge_label = _challenge_readable_label(_institution_level(program), _profile_competitiveness(profile))
...
def _source_warning_v2(program: Program, formal: bool) -> str:
    ...
    if formal:
        return external_note or "关键信息已达到当前系统的正式推荐门槛；递交前仍建议再次打开官网确认。"
    if program.deadline == "NOT_PUBLISHED":
        return external_note or "官网当前季截止日期未发布；先给出材料准备动作，不把它当成正式提交时间。"
    if program.source.field_coverage != "complete":
        return external_note or "目前只定位到项目入口；学费、语言、材料和截止日期还需要打开学校官网逐项核对。"
    return external_note or "这条信息可用于初筛和项目清单草稿；递交前还需要再次核对项目官网页面和申请入口。"
...
def _risks(
    ...
    if not has_program_detail_page(program):
        risks.append("未找到该项目的官方详情页；需要补齐项目页、PDF/FAQ 或网申系统来源后再进入正式时间线。")
    if program.deadline == "NOT_PUBLISHED":
        risks.append("官网当前季截止日期未发布或未核验；时间线会标注往届参考，并给出可执行准备动作。")
...
def _actions(intent: IntentClassification, formal: bool, risks: list[str]) -> list[str]:
    ...
    if not formal:
        actions.insert(0, "打开项目详情页、申请系统或 PDF/FAQ，确认截止日期、学费、语言和材料要求。")
    if risks:
...
def _critical_fields_verified(program: Program) -> bool:
    return has_program_detail_page(program) and program.data_status == DataStatus.verified and program.last_verified_at is not None
...
def _explanation(
    ...
    return RecommendationExplanation(
        hard_condition="通过" if recommendable and data_ready else "需核验" if recommendable else "未通过",
        ...
        timeline_feasibility="可规划" if deadline_known else "准备动作" if program.deadline != "NOT_PUBLISHED" else "待补充",
