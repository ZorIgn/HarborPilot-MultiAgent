...
function RuntimeTraceList({ events, expandedEventId, onToggle, loading }: { events: RuntimeTraceEvent[]; expandedEventId: string | null; onToggle: (eventId: string) => void; loading: boolean }) {
  ...
  return <ol className="runtime-trace-list">{events.map((event) => {
    ...
    </button>{expanded ? <div className="runtime-trace-detail"><dl><div><dt>事件</dt><dd>{event.event_type}</dd></div><div><dt>模型</dt><dd>{event.model ?? "—"}{event.provider ? ` · ${event.provider}` : ""}</dd></div><div><dt>Token</dt><dd>{event.total_tokens?.toLocaleString("zh-CN") ?? "—"}</dd></div><div><dt>成本</dt><dd>{formatCost(event.cost_usd)}</dd></div>{event.execution_id ? <div><dt>Execution ID</dt><dd>{event.execution_id}</dd></div> : null}{event.trace_id ? <div><dt>Trace ID</dt><dd>{event.trace_id}</dd></div> : null}</dl>{event.input_summary ? <p><strong>输入摘要</strong>{event.input_summary}</p> : null}{event.output_summary ? <p><strong>输出摘要</strong>{event.output_summary}</p> : null}{event.error_message ? <p className="trace-error"><strong>{event.error_type ?? "错误"}</strong>{event.error_message}</p> : null}</div> : null}</li>;
  })}</ol>;
}
